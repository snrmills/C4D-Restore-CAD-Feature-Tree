...
#  Updated: 2026-04-14 (v2.0) — Python 3 / C4D 2026 compatibility
#
#  Scenario
#
...
import re
import time
import configparser  # Python 3: was ConfigParser in Python 2
import c4d
...
class Helpers(object):
    ...
    def groupC4dObjects(name, objs):
        ...
        if not isinstance(objs, list):
            objs = [objs]
        for o in objs:
            Helpers.select(o)  # fixed: was bare select(o) in original
        if DEBUG:
            print("creating group %s" % name)
        c4d.CallCommand(100004772)  # group objects
        doc = documents.GetActiveDocument()  # fixed: doc was undefined in original
    ...
    def getActiveObjects(doc):
        """Same as BaseDocument.GetActiveObjects(), except that filters for geometry objects only."""
        lst = []
    ...
    def createObject(typ, name, undo=True):
        ...
        try:
            ...
            if undo is True:
                doc.AddUndo(c4d.UNDOTYPE_NEWOBJ, obj)  # updated: was UNDOTYPE_NEW
            c4d.EventAdd()
        except Exception as e:
            print("*** Caught Exception: %r ***" % e)
        return obj
    ...
    def insertObjectsIntoGroup(objs, grp=None, copy=False):
        ...
        if DEBUG:
            print("inserting objs into group '%s'" % grp.GetName())
...
class RestoreCADFeatureTreeScript(object):
    """The Python script that gets executed when the user hits the Do It! button."""
    ...
    def createC4dGroupsByTreeMerging(self, selection, regex):
        ...
        for op in sel:
            ...
            for part in parts[:-1]:
                ...
                if pathstr not in tree:  # updated: was tree.has_key(pathstr)
                    gname = part
    ...
    def run(self):
        ...
        # updated: GetSelection() replaced with GetActiveObjects() for C4D 2026
...
class RestoreCADFeatureTreeDialog(gui.GeDialog):
    ...
    def CreateLayout(self):
        ...
        # Wrapper Group
        self.GroupBegin(IDC_GROUP_WRAPPER, c4d.BFH_SCALEFIT | c4d.BFV_SCALEFIT, 1, 0, "", 0)
        self.GroupBorder(c4d.BORDER_NONE)
        self.GroupBorderSpace(10, 0, 10, 0)

        # Settings Group
        self.GroupBegin(IDC_GROUP_SETTINGS, c4d.BFH_SCALEFIT | c4d.BFV_SCALEFIT, 2, 0, IDS_STATIC1, 0)
        self.GroupBorder(c4d.BORDER_GROUP_TOP)
        self.GroupBorderSpace(20, 7, 20, 5)
        self.GroupSpace(4, 4)
        self.AddStaticText(IDC_STATIC_SPLIT_REGEX, c4d.BFH_CENTER | c4d.BFV_CENTER, name=IDS_STATIC)
        self.AddMultiLineEditText(IDC_EDIT_SPLIT_REGEX, c4d.BFH_SCALEFIT | c4d.BFV_TOP, initw=374, inith=22,
                                  style=c4d.DR_MULTILINE_MONOSPACED | c4d.DR_MULTILINE_SYNTAXCOLOR | c4d.DR_MULTILINE_PYTHON)

        self.AddStaticText(IDC_STATIC_NAMEFIXES, c4d.BFH_CENTER | c4d.BFV_CENTER, name=IDS_STATIC3)
        self.AddMultiLineEditText(IDC_EDIT_NAME_FIXES, c4d.BFH_SCALEFIT | c4d.BFV_SCALEFIT, initw=390, inith=80,
                                  style=c4d.DR_MULTILINE_MONOSPACED | c4d.DR_MULTILINE_SYNTAXCOLOR | c4d.DR_MULTILINE_PYTHON | c4d.DR_MULTILINE_STATUSBAR)
        ...
        self.GroupSpace(4, 4)
        self.AddButton(IDC_BUTTON_CANCEL, c4d.BFH_LEFT | c4d.BFV_TOP, name=IDS_BUTTON1)
        self.AddButton(IDC_BUTTON_DOIT, c4d.BFH_LEFT | c4d.BFV_TOP, name=IDS_BUTTON)
        self.GroupEnd()
        # Buttons Group End
